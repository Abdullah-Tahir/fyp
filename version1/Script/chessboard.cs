...

{
    ...
    private void Update()
    {
        ...
        RaycastHit info;
        Ray ray = currentCamera.ScreenPointToRay(OVRInput.Get(OVRInput.RawAxis2D.RThumbstick));
        if (Physics.Raycast(ray, out info, Mathf.Infinity, LayerMask.GetMask("Tile", "Hover", "Highlight")))
        {
            // Get the indexes of the tile i've hit
            Vector2Int hitPosition = LookupTileIndex(info.transform.gameObject);
            Debug.Log("rayy");
            //Debug.Log(OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, OVRInput.Controller.Touch));
            ...
            if (currentlyDragging != null && OVRInput.Get(OVRInput.Axis1D.SecondaryIndexTrigger) > 0)
            {
                Debug.Log("if 4");
            ...
            else
            {
                ...
                if (currentlyDragging && OVRInput.Get(OVRInput.Axis1D.SecondaryHandTrigger, OVRInput.Controller.Touch) > 0)
                {
            ...
            if (currentlyDragging)
            {
                ...
                if (horizontalPlane.Raycast(new Ray(transform.position, transform.forward), out distance))
                {
                    currentlyDragging.SetPosition(transform.position + transform.forward * distance + Vector3.up * dragOffset);
                }
            }
